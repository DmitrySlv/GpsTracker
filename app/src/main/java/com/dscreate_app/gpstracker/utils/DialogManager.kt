...
object DialogManager {
    ...
    fun showSaveDialog(context: Context,item: TrackItem?, listener: Listener) {
        ...
        val dialog = builder.create()
        with(binding) {
            val time = item?.time + PROBEL + context.getString(R.string.minutes)
            val speed = context.getString(R.string.speed_tv) + PROBEL + item?.speed + PROBEL +
                    context.getString(R.string.meter_in_sec)
            val distance = context.getString(R.string.distance_tv) + PROBEL + item?.distance +
                    PROBEL + context.getString(R.string.distance_in_kilometer)
            tvTime.text = time
            tvSpeed.text = speed
